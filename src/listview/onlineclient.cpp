    ...
    QFuture<void> future = QtConcurrent::run([=]()
    {
        ...
        if(!isExists && !isExistsHtml)
        {
            ...
            if(bDown && QFileInfo(saveFile).exists())
            {
                ...
                Q_EMIT sigSendData(QByteArrayLiteral("VIDEO_COUNT_ADD|") + m_currentMd5.toLatin1());
            }
...
{
    m_isRecommd = false;
    QByteArray str = QByteArrayLiteral("GET_VIDEO_LIST|") + ui->edit_search->text().toUtf8() + QByteArrayLiteral("|1");
...
{
    Q_EMIT sigSendData(QByteArrayLiteral("GET_VIDEO_RECOMMEND|1"));
}
...
void OnlineClient::on_btn_to_clicked()
{
    ...
    if(page <= total && page > 0)
    {
        if(m_isRecommd)
        {
            QByteArray str = QByteArrayLiteral("GET_VIDEO_RECOMMEND|") + QString::number(page).toUtf8();
            Q_EMIT sigSendData(str);
        }
        else
        {
            QByteArray str = QByteArrayLiteral("GET_VIDEO_LIST|") + m_searchString.toUtf8() + QByteArrayLiteral("|") + QString::number(page).toUtf8();
            Q_EMIT sigSendData(str);
        }
...
void OnlineClient::delayedPageFunction()
{
    if(m_isRecommd)
    {
        QByteArray str = QByteArrayLiteral("GET_VIDEO_RECOMMEND|") + QString::number(m_currentPage).toUtf8();
        Q_EMIT sigSendData(str);
    }
    else
    {
        QByteArray str = QByteArrayLiteral("GET_VIDEO_LIST|") + m_searchString.toUtf8() + QByteArrayLiteral("|") + QString::number(m_currentPage).toUtf8();
        Q_EMIT sigSendData(str);
    }
}
...
void OnlineClient::downloadFinished(const DownloadInfo &data)
{
    ...
    if(data.bDownloaded && QFileInfo(data.outputFilePath).exists())
    {
        ...
        Q_EMIT sigSendData(QByteArrayLiteral("VIDEO_COUNT_ADD|") + strExtra.toLatin1());
    }
    else if(QFileInfo(saveDir+"/fantascene/"+m_currentMd5+"/"+ newName).exists())
    {
        dApp->setWallPaper(saveDir+"/fantascene/"+m_currentMd5+"/"+ newName);
